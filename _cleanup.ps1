...
# 巨型目录：从索引移除（保留磁盘），随后物理移入归档
& git rm -r --cached --quiet quantitative-trading-skills 2>&1 | Out-Null
Write-Host "untracked giant dir from index (exit=$LASTEXITCODE)"
Move-Item -Force quantitative-trading-skills "$dest/quantitative-trading-skills"
Write-Host "moved giant dir into archive"
