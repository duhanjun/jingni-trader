...
```bash
# 数据采集 + 因子构建 + 回测 + 报告
python3 -m jingnitrader.scripts.engine -i "帮我用近3年A股数据做一个20日反转因子选股回测"

# 仅生成报告
python3 -m jingnitrader.scripts.engine -i "生成上个月实盘绩效报告"

# 组合优化
python3 -m jingnitrader.scripts.engine -i "优化当前组合，最大回撤控制在15%以内"
```
...
│   └── SKILL.md
│
├── a-share-factor-engine/         # 因子计算引擎
│   ├── scripts/
│   │   ├── adapters/           # 因子计算器
│   │   │   ├── pandas_ta_calculator.py
│   │   │   └── talib_calculator.py
│   │   ├── engine.py
│   │   └── config.py
│   └── SKILL.md
...
### 2. 因子计算引擎 (a-share-factor-engine)
...
1. 在 `a-share-data-engine/scripts/adapters/` 下创建新适配器
...
- 项目主页：https://github.com/your-username/jingnitrader
- 问题反馈：https://github.com/your-username/jingnitrader/issues
