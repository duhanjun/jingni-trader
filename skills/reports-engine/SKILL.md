...
name: reports-engine
version: 1.0.0
description: A股绩效归因与可视化报告引擎。支持两种投资者模式：量化投资者生成包含净值曲线、绩效指标 TearSheet、月度收益热力图、申万行业归因、Brinson 分解、风格暴露分析的综合 HTML 报告；非量化投资者生成个股技术面深度分析（9 章节，含A股特色：资金面、龙虎榜）和基本面深度分析（10 章节，含A股特色：股东结构）独立报告。集成 TradingView 轻量级 K 线图（8 种可切换技术指标），支持 LLM 占位符注入。基于 QuantStats / Plotly / Matplotlib 输出交互式图表和结构化数据 JSON。
author: quant-team
license: MIT
tags:
  - quant-trading
...
reports-engine 是 A 股量化投研的**绩效归因与可视化报告引擎**，支持两种投资者模式：
