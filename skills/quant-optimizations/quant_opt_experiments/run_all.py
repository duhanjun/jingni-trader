...
def main():
    ...
    if "error" in perf2:
        ...
        # 仍然跑矢量化版本单独
        from skills.quant-optimizations.quant_opt_experiments.vectorized_backtest import vectorized_backtest_multi
        from skills.quant-optimizations.quant_opt_experiments.tests.fixtures import make_synthetic_panel as _ms
        import time as _t
        factor = -close.pct_change(5)
        from skills.quant-optimizations.quant_opt_experiments.vectorized_backtest import rank_topk_signals
        entries, exits = rank_topk_signals(factor, top_pct=0.34, hold_days=20)
        t0 = _t.time()
